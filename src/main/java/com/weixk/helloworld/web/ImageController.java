...
public class ImageController {
    ...
    public Result<String> uploadMultiImg(HttpServletRequest request) {
        ...
        for (MultipartFile file : files) {
            ...
            try {
                ...
                return new Result<>(0, "上传图片失败", e.getMessage());
            }
        }
        return new Result<>(1, "上传图片成功", builder.toString());
    }
    ...
    public Result<String> uploadImg(@RequestParam(value = "file") MultipartFile file) throws IOException {
        if (!file.isEmpty()) {
            ...
            outputStream.flush();
            outputStream.close();
            return new Result<>(1, "图片上传成功", SERVER + "/img/download/" + file.getOriginalFilename());
        }
